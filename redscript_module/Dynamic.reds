...
@addMethod(IronsightGameController)
private cb func FrameGenGhostingFixVehicleWeaponEvent() -> Bool {
	
  // LogChannel(n"DEBUG", s"V has a weapon in their right hand while in a vehicle!");
...
@addMethod(DriveEvents)
public final func VehicleStationaryDumboDeactivation(scriptInterface: ref<StateGameScriptInterface>) -> Void {
...
@addMethod(DriveEvents)
public final func BikeStationaryELGEditorContext(scriptInterface: ref<StateGameScriptInterface>) -> Void {
  ...
  switch(this.m_vehicleCurrentType) {
    case gamedataVehicleType.Bike:
      if NotEquals(RoundTo(this.m_vehicleCurrentSpeed,1),0.0) {
        ...
        this.VehicleStationaryDumboDeactivation(scriptInterface);
        this.BikeStationaryELGEditorContext(scriptInterface);
      }
      break;
