...
//FrameGen Ghosting 'Fix' 2.0 for FSR3 FG Mods, 2024 gramern (scz_g) 2024
...
private cb func OnFrameGenGhostingFixDumboCameraTPPCarEvent(evt: ref<FrameGenGhostingFixDumboCameraTPPCarEvent>) -> Bool {
  ...
  if dumbo4.GetOpacity() != 0.0500000007 {
    dumbo4.SetOpacity(0.0500000007);
  }
  if dumbo5.GetOpacity() != 0.0500000007 {
    dumbo5.SetOpacity(0.0500000007);
  }
...
private cb func OnFrameGenGhostingFixFPPBikeELGSettingsEvent(evt: ref<FrameGenGhostingFixFPPBikeELGSettingsEvent>) -> Void {
  ...
  this.OnFrameGenGhostingFixDumboCameraFPPBikeEvent(950.0, 1800.0, 800.0, 0.0500000007, 0.0500000007, 0.0199999996);
}
...
public final func VehicleStationaryDumboDeactivation(scriptInterface: ref<StateGameScriptInterface>, done: Bool) -> Void {
  let vehicleDumboDeactivation: ref<FrameGenGhostingFixDumboDeActivationVehicleEvent>;

  vehicleDumboDeactivation = new FrameGenGhostingFixDumboDeActivationVehicleEvent();
  ...
  switch(this.m_vehicleCurrentType) {
    case gamedataVehicleType.Bike:
      if NotEquals(RoundTo(this.m_vehicleCurrentSpeed,1),0.0) {
        if RoundTo(this.m_vehicleCurrentSpeed,1)<5.0 {
          this.BikeCameraChangeSlow(scriptInterface, this.m_bikeCameraContext);
        } else {
          this.BikeCameraChange(scriptInterface, this.m_bikeCameraContext);
        }
      } else {
...
// Setting dumbos when exiting a vehicle---------------------------------------------------------------------------------------
@addMethod(IronsightGameController)
private cb func OnUnmountingEvent(evt: ref<UnmountingEvent>) -> Bool {
  ...
  let dynamic_dumbo2: ref<inkWidget> = root.GetWidgetByPathName(n"dynamic_dumbo2") as inkWidget;
  let dynamic_dumbo3: ref<inkWidget> = root.GetWidgetByPathName(n"dynamic_dumbo3") as inkWidget;
  let dumbo1_vehicle: ref<inkWidget> = root.GetWidgetByPathName(n"dumbo1_vehicle") as inkWidget;
  let sdvignette: ref<inkWidget> = root.GetWidgetByPathName(n"sdvignette") as inkWidget;


  dynamic_dumbo1.SetOpacity(0.00100000005);
  dynamic_dumbo2.SetOpacity(0.00100000005);
  dynamic_dumbo3.SetOpacity(0.00100000005);
  dumbo1_vehicle.SetOpacity(0.0);
  sdvignette.SetOpacity(0.00100000005);

  dynamic_dumbo2.SetSize(500, 2160);
  dynamic_dumbo2.Reparent(root);
}