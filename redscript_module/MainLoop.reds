//FrameGen Ghosting 'Fix' 4.0.0, 2024 gramern (scz_g) 2024

@addField(IronsightGameController) public let m_cameraForward: Vector4;

//Camera data---------------------------------------------------------------------------------------
@addMethod(IronsightGameController)
public func FrameGenGhostingFixGetCameraData() -> Void {
  let playerPuppet: ref<GameObject> = this.GetPlayerControlledObject();
  let targetingSystem: ref<TargetingSystem> = GameInstance.GetTargetingSystem(playerPuppet.GetGame());
  let cameraPosition: Vector4;
  let cameraForward: Vector4;
  targetingSystem.GetCrosshairData(playerPuppet, cameraPosition, cameraForward);

  this.m_cameraForward = cameraForward;

  // LogChannel(n"DEBUG", s"\(cameraForward)");
}
...
public class FrameGenGhostingFixLoopCallback extends DelayCallback {
  ...
  public func Call() -> Void {
    ...
    this.ironsightController.FrameGenGhostingFixGetCameraData();
